"""公文格式校验引擎：程序规则校验 + AI 辅助判断 + 自动修正。

设计要点（与旧系统一致）：
1. 程序规则校验（确定性）：用 python-docx 读取字体/字号/对齐/行距/缩进/页边距等
   真实排版属性，与 FormatRule 配置的期望值逐项比对。
2. AI 辅助判断：只处理规则难以表达的问题，source 字段区分 "rule" / "ai"。
3. 自动修正：apply_fixes 按 issue 中的 fix_hint 和 paragraph_index 回写 docx。
4. 规则不写死：所有期望值来自数据库 FormatRule。

引擎本身不依赖 FastAPI / 数据库；AI 调用通过 WritingAssistant 注入。
"""
...
class FormatCheckEngine:
    """格式校验引擎：规则校验 + AI 辅助 + 自动修正。"""

    def __init__(self, assistant: Optional[WritingAssistant] = None) -> None:
    ...
    def _check_docx(self, file_path: Path, filename: str, rules: List[Dict],
        ...
        # ---- AI 辅助 ----
        ai_used = False
        if use_ai and self.assistant is not None:
            full_text = "\n".join(p.text for p in paragraphs if p.text.strip())
            ai_issues = self._ai_check(full_text[:settings.FORMAT_CHECK_AI_BUDGET], rules)
    ...
    def _check_plain_text(self, text: str, filename: str, rules: List[Dict],
        ...
        ai_used = False
        if use_ai and self.assistant is not None:
            ai_issues = self._ai_check(text[:settings.FORMAT_CHECK_AI_BUDGET], rules)
            if ai_issues is not None:
                issues.extend(ai_issues)
                ai_used = True
    ...
    def _ai_check(self, text: str, rules: List[Dict]) -> Optional[List[Dict]]:
        ...
        if rules:
            scope = """请只检查这些规则难以判断的问题：
1. 落款（发文机关署名）和成文日期是否缺失或明显位置不当；
2. 标题与正文结构是否明显混乱；
3. 是否存在明显的重复段落、残留占位符（如"××"、"XXX"、"待补充"）；
4. 文号格式是否明显不规范（若有文号）。"""
        else:
            # 无规则模式：AI 承担完整格式/规范审查（参照党政机关公文格式惯例）
            scope = """当前系统尚未配置格式规则，请你参照党政机关公文格式惯例做全面审查：
1. 标题、主送、正文、落款（发文机关署名）、成文日期等要素是否缺失或位置明显不当；
2. 结构层次是否清晰（"一、""（一）""1."层级是否混乱）；
3. 是否存在重复段落、残留占位符（如"××"、"XXX"、"待补充"）、明显口语化表述；
4. 文号格式是否明显不规范（若有文号）；
5. 其他明显不符合公文规范的问题。"""
    ...
    def apply_fixes(self, file_path: Path, issues: List[Dict],
        ...
        for idx in accepted_indices:
            ...
            try:
                ...
                logger.warning("修正第%s条失败（已跳过）: %s", idx, e)
        ...
        for p_idx in sorted(set(deletes), reverse=True):
            try:
                ...
                logger.warning("删除第%s段失败（已跳过）: %s", p_idx + 1, e)
