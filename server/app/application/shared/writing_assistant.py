"""写作助手：提示词编排层（移植自旧 services/llm_service.py）。

只依赖注入的 LLMGateway 端口，不感知具体模型/传输。
所有提示词规则与旧系统保持一致。
"""
...
class WritingAssistant:
    ...
    def _load_prompts(self) -> Dict:
        ...
        if _PROMPT_DIR.exists():
            for category_dir in _PROMPT_DIR.iterdir():
                if category_dir.is_dir():
                    for file in category_dir.glob("*.jinja2"):
                        prompts[f"{category_dir.name}/{file.stem}"] = file.read_text(encoding="utf-8")
        return prompts
    ...
    def complete(self, messages: List[Dict], temperature: float = 0.7, max_tokens: int = 4096) -> str:
        """裸调用网关（供 format_check / ppt / workflow 等模块使用）。"""
        return self.gateway.complete(messages, temperature=temperature, max_tokens=max_tokens)
    ...
    def chat(self, message: str, history: List[Dict], sources: List[Dict], user_role: str,
        ...
        """主对话入口（签名与旧 LLMService.chat 完全一致）。"""
        ...
        return self.gateway.complete(messages)

    def generate_document(self, doc_type: str, topic: str, references: str = "",
                          style: str = "严谨", word_count: int = 1000) -> str:
        ...
        return self.gateway.complete(messages, max_tokens=8192)
