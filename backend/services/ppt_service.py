# -*- coding: utf-8 -*-
"""智能 PPT 服务 V2（覆盖 backend/services/ppt_service.py）

核心设计：内容与版式分离
- 文档 outline 只保存内容：页面类型、标题、要点、结构化数据块、图片名；
- 模板（PPTTemplate）保存视觉风格（colors/font）+ 版式体系（layouts：
  每种页型一个版式变体）。同一份内容换模板 = 换渲染样式，内容不动。

页型体系（10 种）：
  cover 封面 / toc 目录 / section 章节页 / content 正文（可选配图）
  data 数据卡片页 / chart 图表页（导出为原生可编辑图表）
  case 案例页 / timeline 时间轴 / process 流程图 / summary 总结页 / closing 结束页

模板版式变体（layouts 字典的值，渲染器据此排版）：
  cover:   band_bottom（标题居中+底部色带） / left_block（左侧色块）
  toc:     numbered_list（编号列表） / cards（卡片网格）
  section: left_block（左侧色块） / center（居中大标题）
  content: bar_title（标题侧条） / top_band（顶部色带）
  closing: center / brand_band

导出：python-pptx 手工排版，文本/形状/图表均为可编辑元素（需求 7）。
...
def generate_outline(llm_service, source_type: str, topic: str, content: str,
...
def ai_slide_action(llm_service, action: str, slide: Dict, instruction: str = "") -> Dict:
...
def ai_generate_visual(llm_service, kind: str, slide: Dict, instruction: str = "") -> Dict:
...
def ai_structure_action(llm_service, action: str, slides: List[Dict], index: int,
...
只输出新增页面的 JSON（type/title/points/blocks 结构，内容要实，不要虚构数据）。"""
        raw = llm_service._call_vllm([{"role": "system", "content": _SLIDE_SYSTEM},
                                      {"role": "user", "content": prompt}],
...
def extract_style_from_pptx(file_bytes: bytes) -> Dict:
    ...
    版式体系沿用默认变体（生成时以原 pptx 为底版，母版视觉完整保留）。
    """
    import zipfile
    colors = {}
    font = "微软雅黑"
        ...
        with zipfile.ZipFile(io.BytesIO(file_bytes)) as z:
            ...
            if theme_files:
                ...
                def _clr(tag, xml_text):
                    # 主题色可能是 srgbClr val= 或 sysClr lastClr=
...
def analyze_template(file_bytes: bytes) -> Dict:
    ...
    返回 {
      slide_size: {w, h}, colors, font,
      layouts: [{id, name, detected_type, source_slide_index, slide_size,
                 background, element_schema, text_regions, image_regions,
                 chart_regions, shape_regions, placeholders}]
    }
    解析失败时返回空 layouts，调用方回退 DEFAULT_LAYOUTS。
    """
    from pptx import Presentation
...
def _fill_text_region(shape, lines, keep_para_format=True):
    ...
    if not paras:
        return
    proto = paras[0]._p  # 格式模板段落
    # 先清空全部段落（保留第一个）
    for p in list(tf.paragraphs[1:]):
        p._p.getparent().remove(p._p)
    first = tf.paragraphs[0]
    # 清空第一个段落的 runs，保留 pPr
...
def _fill_learned_slide(slide, layout, sdata, colors, font, sw, sh, prs):
    ...
    # 用 schema 的 role 信息标注克隆页形状（按位置+类型匹配）
    def role_of(e):
        for se in schema:
...
def build_pptx_learned(outline: Dict, template: Dict, base_pptx: bytes,
    ...
    # 先按方案克隆（从尾部开始克隆避免索引变化？add_slide 追加不影响源页）
    new_slides = []
    for s, lay, src in plan:
        try:
            ...
            print(f"[PPT] 克隆页面失败: {e}")
    # 删除模板原始页面（连关系一起删，避免示例内容残留在文件中）
